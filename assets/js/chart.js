...
export function weightChart(dog, weights, { h = 180, months = 12 } = {}) {
  ...
  if (!pred || !weights.length) return `<div class="empty"><div class="big">⚖️</div>${t('Добавьте вес, чтобы увидеть кривую роста')}</div>`;
  ...
  return `<svg class="chart" viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" role="img"
      aria-label="${t('График веса с коридором нормы')}">
    ...
    <text x="2" y="${y(maxKg * 0.9) + 4}" font-size="9" fill="var(--text-3)">${t('{kg} кг', { kg:Math.round(maxKg * 0.9) })}</text>
    <text x="2" y="${h - pad.b + 2}" font-size="9" fill="var(--text-3)">0</text>
    <text x="${pad.l}" y="${h - 4}" font-size="9" fill="var(--text-3)">${t('{n} нед.', { n:Math.round(minW) })}</text>
    <text x="${w - 46}" y="${h - 4}" font-size="9" fill="var(--text-3)">${t('{n} нед.', { n:Math.round(maxW) })}</text>
  </svg>`;
}
